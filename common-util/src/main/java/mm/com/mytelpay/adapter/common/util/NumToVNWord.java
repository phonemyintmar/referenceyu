...
public final class NumToVNWord {
    ...
    // Static
    private static final List<String> digitsName =
            Arrays.asList("không", "một", "hai", "ba", "bốn", "năm", "sáu", "bảy", "tám", "chín");
    private static final List<String> thousandsName =
            Arrays.asList("", "nghìn", "triệu", "tỷ", "nghìn tỷ", "triệu tỷ", "tỷ tỷ");
    ...
    private static List<Integer> stringToInt(String triplet) {
        return triplet.chars().map(NumToVNWord::charToInt).boxed().collect(Collectors.toList());
    }
    ...
    public static String num2String(long num) {
        ...
        String result =
                groupOfThousand.stream()
                        ...
                        .map(
                                vnString ->
                                        vnString.trim().isEmpty()
                                                ? ""
                                                : vnString
                                                        + " "
                                                        + thousandsName.get(
                                                                groupOfThousand.size()
                                                                        - 1
                                                                        - index.get()))
     ...
     *
     * <p>Explain: count the amount of consecutive "000" at the end of the number and compare it
     * with number length
     *
