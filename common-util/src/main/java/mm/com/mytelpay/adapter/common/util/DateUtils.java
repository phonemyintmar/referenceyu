...
public final class DateUtils {
    ...
    public static Date parse(String dateString, String format) {
        try {
            ...
            log.error(createLogMessage(dateString, format, null), e);
        }
        return null;
    }
    ...
    public static Date parseDate(String dateString) {
        try {
            ...
            log.error(createLogMessage(dateString, NORM_DATE_PATTERN, NORM_2_DATE_PATTERN), e);
        }

        try {
            return new SimpleDateFormat(NORM_2_DATE_PATTERN).parse(dateString);
        } catch (ParseException e) {
            log.error(createLogMessage(dateString, NORM_2_DATE_PATTERN, null), e);
        }
        return null;
    }
    ...
    public static Date parseTime(String timeString) {
        try {
            ...
            log.error(createLogMessage(timeString, NORM_TIME_PATTERN, null), e);
        }
        return null;
    }
    ...
    public static Date parse(String dateStr) {
        ...
        try {
            ...
            log.error(createLogMessage(dateStr, null, null), e);
        }
        return null;
    }
    ...
    public static Instant getTimeStart(String dateText) {
        LocalDateTime date =
                LocalDateTime.parse(
                        dateText + " 00:00:00", DateTimeFormatter.ofPattern(NORM_DATETIME_PATTERN));
    ...
    public static Instant getTimeFinish(String dateText) {
        LocalDateTime date =
                LocalDateTime.parse(
                        dateText + " 23:59:59", DateTimeFormatter.ofPattern(NORM_DATETIME_PATTERN));
        return date.toInstant(ZoneOffset.ofHours(+7));
    }
    ...
    public static Integer getDayCount(String start, String end) {
        ...
        try {
            ...
            // time is always 00:00:00, so rounding should help to ignore the missing hour when
            // going from winter to summer time, as well as the extra hour in the other direction
            diff =
                    Math.toIntExact(
                            Math.round(
                                    (dateEnd.getTime() - dateStart.getTime()) / (double) 86400000));
        } catch (Exception e) {
    ...
    private static String createLogMessage(String stringToFormat, String format, String format2) {
        if (format == null && format2 == null) {
            return "Parse " + stringToFormat + " with format normal error!";
        }
        if (format2 == null) {
            return "Parse " + stringToFormat + " with format " + format + " error!";
        } else {
            return "Parse "
                    + stringToFormat
                    + " with format "
