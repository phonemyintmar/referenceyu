package mm.com.mytelpay.adapter.common.mapper.util;

import mm.com.mytelpay.adapter.common.dto.request.PagingRequest;
        ...
        if (StringUtils.hasLength(criteria.getSortBy())) {
            ...
            if (StringUtils.hasLength(sortStr)) {
                if (sortStr.contains(".")) {
                    ...
                    if (sortStr.endsWith(PagingRequest.DESC_SYMBOL)
                            && StringUtils.hasLength(sortParams[0])) {
                        orders.add(Sort.Order.desc(sortParams[0]));
                    } else if (sortStr.endsWith(PagingRequest.ASC_SYMBOL)
                            && StringUtils.hasLength(sortParams[0])) {
